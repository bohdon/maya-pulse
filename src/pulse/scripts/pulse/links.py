...
def unlink(node):
    ...
    """
    meta.remove_metadata(node, class_name=LINK_METACLASS)
...
def get_link_meta_data(node):
    ...
    """
    result = meta.get_metadata(node, class_name=LINK_METACLASS)
    if not result:
        return {}
...
def set_link_meta_data(node, link_data):
    ...
    """
    meta.set_metadata(node, class_name=LINK_METACLASS, data=link_data)
...
def get_all_linked_nodes():
    ...
    """
    return meta.find_meta_nodes(class_name=LINK_METACLASS)
...
def cleanup_links():
    ...
    """
    link_nodes = meta.find_meta_nodes(class_name=LINK_METACLASS)

...
def install_main_menu():
    ...
    from .anim.tools import AnimToolsWindow
    ...
    pm.menuItem(parent=anim_menu_id, label="Anim Tools", command=_cmd(AnimToolsWindow.toggleWindow))
...
def _create_menu(label: str, menu_id: str):
    ...
    if pm.menu(menu_id, query=True, exists=True):
        pm.deleteUI(menu_id, menu=True)
